...
def certified_target(t):
    ...
    edges=[(i,j,A[i][j]) for i in range(n) for j in range(i) if A[i][j]]
    for size in range(1,n//2+1):
        ...
        solver.add(z3.PbLe([(z3.Xor(b[i],b[j]),w) for i,j,w in edges],bound))
        ...
        weight=sum(w for i,j,w in edges if (i in S)!=(j in S))
